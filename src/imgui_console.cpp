...
// The following three functions (InputTextCallback_UserData, InputTextCallback,
// InputText) are obtained from misc/cpp/imgui_stdlib.h Which are licensed under
...
ImGuiConsole::ImGuiConsole(std::string c_name, size_t inputBufferSize,
  ...
  // Check if there's initialized system.
  if (system) {
    m_ConsoleSystem = system;
    m_UseInternalSystem = false;
  } else {
    m_ConsoleSystem = new csys::System;
    m_UseInternalSystem = true;
  }
...
ImGuiConsole::~ImGuiConsole() {
  if (m_UseInternalSystem) {
    delete m_ConsoleSystem;
  }
}
...
void ImGuiConsole::LogWindow() {
  ...
  if (ImGui::BeginChild("ScrollRegion##", ImVec2(0, -footerHeightToReserve),
                        false, 0)) {
    // Display colored command output.
    static const float timestamp_width =
        ImGui::CalcTextSize("00:00:00:0000").x;  // Timestamp.
      ...
      if (item.m_Type == csys::COMMAND) {
        if (m_TimeStamps)
          ImGui::PushTextWrapPos(
              ...
              timestamp_width);  // Wrap before timestamps start.
        if (count++ != 0)
          ImGui::Dummy(ImVec2(
              -1, ImGui::GetFontSize()));  // No space for the first command.
      }
    ...
    // Auto-scroll logs.
    if ((m_ScrollToBottom &&
         (ImGui::GetScrollY() >= ImGui::GetScrollMaxY() || m_AutoScroll)))
...
void ImGuiConsole::InputBar() {
  ...
  if (ImGui::InputText("Input", &m_Buffer, inputTextFlags, InputCallback,
                       this)) {
    // Validate.
    if (!m_Buffer.empty()) {
...
void ImGuiConsole::MenuBar() {
  if (ImGui::BeginMenuBar()) {
    ...
    if (ImGui::BeginMenu("Appearance")) {
      ...
      ImGui::Indent();
      ImGui::ColorEdit4("Command##", (float *)&m_ColorPalette[COL_COMMAND],
                        flags);
      ImGui::ColorEdit4("Log##", (float *)&m_ColorPalette[COL_LOG], flags);
      ImGui::ColorEdit4("Warning##", (float *)&m_ColorPalette[COL_WARNING],
                        flags);
      ImGui::ColorEdit4("Error##", (float *)&m_ColorPalette[COL_ERROR], flags);
      ImGui::ColorEdit4("Info##", (float *)&m_ColorPalette[COL_INFO], flags);
      ImGui::ColorEdit4("Time Stamp##", (float *)&m_ColorPalette[COL_TIMESTAMP],
                        flags);
      ImGui::Unindent();
    ...
    case ImGuiInputTextFlags_CallbackCompletion: {
      ...
      if (!trim_str.empty()) {
        ...
        if (!console->m_CmdSuggestions.empty()) {
          console->m_ConsoleSystem->Log(csys::COMMAND)
              << "Suggestions: " << csys::endl;

          for (const auto &suggestion : console->m_CmdSuggestions)
            console->m_ConsoleSystem->Log(csys::LOG)
                << suggestion << csys::endl;
        ...
        std::string partial = console_autocomplete->Suggestions(
            trim_str.substr(startSubtrPos, endPos + 1),
            console->m_CmdSuggestions);

        // Autocomplete only when one work is available.
        if (!console->m_CmdSuggestions.empty() &&
            console->m_CmdSuggestions.size() == 1) {
    ...
    case ImGuiInputTextFlags_CallbackHistory: {
      ...
      if (console->m_HistoryIndex == std::numeric_limits<size_t>::min())
        console->m_HistoryIndex =
            console->m_ConsoleSystem->History().GetNewIndex();
      ...
      if (data->EventKey == ImGuiKey_UpArrow) {
        ...
        if (console->m_HistoryIndex <
            console->m_ConsoleSystem->History().Size())
          ++(console->m_HistoryIndex);
      }

      // Get history.
      std::string prevCommand =
          console->m_ConsoleSystem->History()[console->m_HistoryIndex];
...
void ImGuiConsole::SettingsHandler_ClearALl(ImGuiContext *ctx,
                                            ImGuiSettingsHandler *handler) {}

void ImGuiConsole::SettingsHandler_ReadInit(ImGuiContext *ctx,
                                            ImGuiSettingsHandler *handler) {}

void *ImGuiConsole::SettingsHandler_ReadOpen(ImGuiContext *ctx,
                                             ImGuiSettingsHandler *handler,
                                             const char *name) {
...
void ImGuiConsole::SettingsHandler_ReadLine(ImGuiContext *ctx,
                                            ImGuiSettingsHandler *handler,
...
void ImGuiConsole::SettingsHandler_ApplyAll(ImGuiContext *ctx,
                                            ImGuiSettingsHandler *handler) {
  if (!handler->UserData) return;
}

void ImGuiConsole::SettingsHandler_WriteAll(ImGuiContext *ctx,
                                            ImGuiSettingsHandler *handler,
                                            ImGuiTextBuffer *buf) {
  if (!handler->UserData) return;
